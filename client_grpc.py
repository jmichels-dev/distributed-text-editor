...
def signinLoop(stub):
    ...
    # Try to sign in new user
        ...
        if helpers_grpc.isValidUsername(username):
            # Remove whitespace
            username = username.strip().lower()
            unreadsOrError = stub.AddUser(texteditor_pb2.Username(name=username))
            eFlag, msg = unreadsOrError.errorFlag, unreadsOrError.unreads
    if eFlag:
        print(msg)
...
def messageLoop(username, stub):
    ...
    if command == 'S' or command == 's':
        ...
        sender = texteditor_pb2.Username(name=username)
        recipient = texteditor_pb2.Username(name=send_to_user)
        payload = texteditor_pb2.Payload(msg=message)
        senderResponse = stub.Send(texteditor_pb2.SendRequest(sender=sender, recipient=recipient, sentMsg=payload))
    ...
    if command == 'D' or command == 'd':
        ...
        if confirm:
            deleteResponse = stub.Delete(texteditor_pb2.Username(name=username))
...
def run():
    ...
    with grpc.insecure_channel('{}:{}'.format(ip, port)) as channel:
        stub = texteditor_pb2_grpc.ChatStub(channel)
